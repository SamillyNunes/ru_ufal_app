...
import 'package:ru_ufal_app/models/food_model.dart';
import 'package:ru_ufal_app/models/meal_model.dart';
import 'package:ru_ufal_app/utils/formatter_util.dart';
import 'package:ru_ufal_app/utils/text_formatter_util.dart';
...
class _HomeViewState extends State<HomeView> {
  ...
  List<String> tags = ["Café da manhã", "Almoço", "Jantar"];
  ...
  Map<String, List<MealModel>> separateInCategories(List<MealModel> listMeal) {
    Map<String, List<MealModel>> categoriesMap = {};

    for (String category in categories) {
      List<MealModel> mealModelListForCategory = [];
      for (MealModel mm in listMeal) {
        if (mm.food.category.toLowerCase() == category.toLowerCase()) {
          mealModelListForCategory.add(mm);
        }
      }
      categoriesMap[category] = mealModelListForCategory;
    }
    return categoriesMap;
  }

  List<List<MealModel>> separateMealsInTypes(List<MealModel> listMeals) {
    DateTime currentDate = DateTime.now();

    // de acordo com o tamanho das tags
    List<List<MealModel>> listMealsInTypes = [[], [], []];

    for (MealModel mm in listMeals) {
      int tagIndex = 0;
      for (String tag in tags) {
        String mealStringConverted =
            TextFormatterUtil.removeAccents(text: mm.type.toLowerCase());

        String tagConverted =
            TextFormatterUtil.removeAccents(text: tag.toLowerCase());
        print("Meal: $mealStringConverted");
        print("Tag: $tagConverted");

        if (mealStringConverted == tagConverted &&
            compareDates(mm.date, currentDate)) {
          listMealsInTypes[tagIndex].add(mm);
          break; // para nao precisar iterar nos proximos tipos
        }
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
        onRefresh: () async {
          setState(() {
            homeController.fetchMenuData();
          });
        },
        child: FutureBuilder(
          future: homeController.fetchMenuData(),
          builder: (context, AsyncSnapshot<List<MealModel>> snapshot) {
            if (snapshot.hasData) {
              List<MealModel> listMeals = snapshot.data!;
              print("list models lenght: ${listMeals.length}");

              List<List<MealModel>> typeMealsList =
                  separateMealsInTypes(listMeals);
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.vertical(
                        bottom: Radius.circular(deviceSize.width * 0.07),
                      ),
                    ),
                    child: Container(
                      // color: Colors.red,
                  ...
                  Expanded(
                    child: ListView(
                      ...
                      children: [
                        ...
                        ValueListenableBuilder(
                          valueListenable: homeController.tagSelectedNotifier,
                          builder: (context, int tagIndex, child) {
                            if (typeMealsList[tagIndex].length >= 1) {
                              print("type list: ${typeMealsList[tagIndex]}");
                              Map<String, List<MealModel>> categoriesMap =
                                  separateInCategories(typeMealsList[tagIndex]);
                              ...
                              return ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  if (categoriesMap[mapKey]!.length == 0) {
                                    return Container();
                                  }
                                  ...
                                  return Column(
                                    ...
                                    children: [
                                      ...
                                      ListView.builder(
                                        ...
                                        physics: NeverScrollableScrollPhysics(),
                                        itemCount:
                                            categoriesMap[mapKey]!.length,
                                        itemBuilder: (context, index) {
                                          FoodModel foodModel =
                                              categoriesMap[mapKey]![index]
                                                  .food;
                                          return FoodTile(
                              ...
                              return Padding(
                                ...
                                child: Center(
                                  child: Text(
                                      "Não tem comidas para essa categoria neste dia."),
                                ),
                              );
                            }
                          },
